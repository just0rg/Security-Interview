...
PHP 的 file_get_contents() 和 cURL 默认支持 HTTP、HTTPS 和 file 协议。
...
彻底解决该漏洞可以分析根因，Fastjson主要因为Autotype导致的、Log4j主要因为Lookup，可考虑移除经常出现问题的代码，制作内部精简定制版本。或者通过应用运行时防护（RASP）。
...
密码策略、多因素、密码哈希加盐、账户锁定

#### 硬编码密钥有何风险以及如何系统解决？

密钥泄露：硬编码密钥容易被逆向工程或代码泄露，导致密钥暴露。

密钥管理困难：密钥一旦泄露，难以快速更换，且多个环境（开发、测试、生产）可能需要不同的密钥。

合规性问题：某些安全标准（如PCI DSS、GDPR）禁止硬编码密钥。

解决：密钥管理系统（KMS）、环境变量动态注入、动态密钥生成、密钥轮换、代码审查与自动化扫描，核心点在于密钥需要和代码分离，尽可能少的让密钥被人接触到。

#### 0day漏洞如何防御

1. 最小权限原则：
   - 限制应用程序和用户的权限，减少漏洞被利用后的影响范围。
2. 深度防御：
   - 采用多层安全防护措施，如防火墙、入侵检测系统（IDS）、Web应用防火墙（WAF）等，增加攻击难度。
3. 及时更新与补丁管理：
   - 定期更新操作系统、库和应用程序，及时应用安全补丁。
4. 漏洞扫描与渗透测试：
   - 定期进行漏洞扫描和渗透测试，发现潜在的安全问题。
5. 沙箱与隔离：
   - 在沙箱环境中运行不受信任的代码，隔离潜在的攻击。
6. 威胁情报：
   - 订阅威胁情报服务，及时了解最新的0day漏洞信息，并采取相应的防护措施。
7. 应急响应计划：
   - 制定并演练应急响应计划，确保在0day漏洞被利用时能够快速响应和修复。

#### GitHub等三方泄漏敏感信息如何体系防御

限制代码出网、代码敏感信息审核、访问控制与权限管理、安全培训

#### 业务逻辑漏洞如何通过技术手段避免写出来？

权限控制与访问控制、安全组件、威胁建模、安全需求分析、安全编码规范

#### 软件供应链后门和漏洞如何系统规避

**供应商评估：**

对软件供应商进行安全评估，确保其遵循安全开发实践。

**代码审查与审计：**

对第三方库和组件进行代码审查和安全审计，确保其没有后门或漏洞。

**依赖管理：**

使用依赖管理工具（如Maven、npm）管理第三方库，并定期更新到最新版本。

**签名与验证：**

对下载的软件包进行签名验证，确保其完整性和真实性。
...
凡是和业务比较贴近的漏洞都算业务逻辑漏洞，比如身份校验相关的风险（未授权访问、非正常账户态、身份可枚举、水平越权、垂直越权等）、接口逻辑实现不一致（不同协议实现不一致、同类产品不同逻辑、不同阶段逻辑不一致-流程绕过）、不安全的可信端数据（APP数据）、预设要求不符合（依赖条件不安全-业务校验属性设计、人工客服容易被骗-业务流程设计）、滥用合理业务需求等等。

#### 哪些账户状态会导致预期外的风险？

当账户状态改变时，由于各个业务逻辑并不一定严格遵循该账户状态的要求导致的还可以继续进行一些预期外的操作。一般账户状态在注销、禁用、风控、司法冻结等时候，会出现该问题。



#### 任意密码重置场景产生原因

绕过正常流程、验证码猜测、逻辑缺陷、用户标识校验失效
...
一般针对数字类型的越权可以通过递增枚举数字，根据响应情况可以判断是否存在越权。

此外还可以通过两个账号相互访问对方创建的资源来判断，可避免不连续的数字导致的遗漏。

#### 通过数字加减遍历或通过两个账号互测的方式进行水平越权测试有何优劣势？

数字加减遍历只能针对数字类型资源ID，且只能针对连续的数字，遗漏情况较多，且对业务有损（会导致操作或查询他人数据）。

通过两个账号的方式，可以不用遍历，不容易被发现，也不会遗漏，对他人数据无影响。

当然，在某些场景下，数字加减遍历方式相较于换账号方式更有优势。比如在多个参数的接口中，有N个参数是有校验和当前账号关系，但有一个参数未校验。此时用换账号的方式就得将多个账号都调整为正确参数后再测试问题参数。而+-方式则可以直接测试每一个参数。
...
1. 强制资源所有权验证：确保每次访问资源时都验证用户权限。
2. 细粒度的访问控制：使用 ABAC 或 RBAC 实现精细的权限管理。
3. 资源隔离与多租户设计：将不同用户的资源完全隔离。
4. 避免直接暴露资源 ID：使用加密或哈希的资源标识符。
5. 强制会话与资源绑定：确保会话与资源绑定，防止会话滥用。
6. 自动化测试与代码审查：通过测试和审查确保权限验证逻辑的正确性。
7. 使用安全的框架和库：避免自行实现复杂的权限控制逻辑。
8. 日志与监控：记录资源访问日志，及时发现异常行为。
9. 教育与培训：提高开发团队的安全意识。
...
由于地层的邮箱系统往往都支持多邮箱语法，传入多个邮箱 “`mail1@baidu.cn;mail2@wufeifei.com“`，可以绕过业务侧限制。
...
| 特性         | 人工代码审计                     | 自动化代码审计                       |
| ------------ | -------------------------------- | ------------------------------------ |
| 覆盖范围     | 深度覆盖，适合复杂逻辑           | 广度覆盖，适合大规模代码库           |
| 漏洞检测能力 | 能发现复杂漏洞（如业务逻辑漏洞） | 擅长检测模式化漏洞（如SQL注入、XSS） |
| 效率与速度   | 效率较低，适合关键模块           | 速度快，适合CI/CD管道                |
| 误报与漏报   | 误报率低，漏报率可能较高         | 误报率高，漏报率可能较高             |
| 成本         | 成本较高，依赖专家经验           | 成本较低，适合大规模扫描             |
| 适用场景     | 关键模块、复杂逻辑               | 大规模代码库、常见漏洞检测           |
| 工具成熟度   | 灵活性强，依赖人工技能           | 工具不断进步，但受限于规则库         |
| 结果可解释性 | 结果详细，附带解释               | 结果快速，但可能缺乏上下文           |
...
4. 运行时漏洞：需要动态分析。
5. 环境相关漏洞：涉及部署环境和配置。
6. 0day漏洞：未知漏洞，缺乏检测规则。
7. 用户交互漏洞：涉及用户行为和界面交互。
8. 加密与密钥管理漏洞：需要密码学知识。
9. XSS的上下文相关漏洞：需要分析输入输出上下文。
10. 社会工程漏洞：涉及人为因素。

#### 密钥的识别的正则如何写？

`\b(?:[A-Za-z0-9+/=]{32,}|AKIA[A-Z0-9]{16}|AIza[A-Za-z0-9\-_]{35}|[A-Fa-f0-9]{32,64}|[A-Za-z0-9\-_]{32,})\b`

#### 正则(a+)+会存在什么风险？

正则表达式 `(a+)+` 的主要风险是灾难性回溯，它会导致：

1. 性能问题：匹配时间随输入长度呈指数级增长。
2. 资源耗尽：可能耗尽 CPU 和内存资源，导致系统崩溃。
3. 拒绝服务攻击：攻击者可以利用此特性发起 DoS 攻击。
...
#### 程序对请求的URL的正则为/^http\:\/\/.*\.baidu.cn($|(\/[^<>\'\"]*))/，如何绕过？
...
| ------------------ | ---------------------------- | ---------------------------- |
| **语法树生成时机** | 编译阶段                     | 运行时                       |
| **语法树优化**     | 深度优化（静态分析）         | 较少优化（动态优化，如 JIT） |
| **语法树持久化**   | 通常不持久化                 | 可能持久化（如字节码）       |
| **语法树复杂度**   | 较复杂（支持静态分析）       | 较简单（快速解析和执行）     |
| **语法树使用场景** | 静态分析、代码生成           | 运行时解析、执行             |
| **工具支持**       | 丰富（调试器、静态分析工具） | 较少（调试器、日志）         |


#### JavaWeb应用中的反序列化漏洞的Source和Sink是什么？

在 Java Web 应用中，反序列化漏洞的 Source 包括 HTTP 请求参数、文件上传、消息队列等，而 Sink 主要是 ObjectInputStream.readObject() 方法或第三方库的反序列化操作
...
1. 识别输入点：包括 URL 参数、表单输入、HTTP 头等。
2. 注入测试 payload：使用常见的 XSS payload 进行测试。
3. 观察输出：检查页面源代码或 DOM 结构，确认是否执行了注入的脚本。
4. 使用工具辅助：借助浏览器插件、自动化扫描工具和 payload 生成工具提高效率。
5. 检测盲 XSS：注入包含外部 URL 的 payload，监控外部服务器的日志。
6. 绕过防御机制：尝试绕过输入过滤和输出编码。

#### 甲方黑盒是否应该有爬取流量功能？

+ 测试目标：是否需要全面覆盖攻击面？
+ 资源限制：是否有足够的计算资源和网络带宽？
+ 合规性要求：是否获得明确授权且不违反隐私法规？

如果条件允许，爬取流量功能可以显著提高测试效率和覆盖率，但需要结合人工测试和合规性要求，确保测试的安全性和有效性。

#### 黑盒如何扫描无法出网的SSRF？

使用内部 URL 测试：尝试访问 localhost、127.0.0.1 等内部资源。

使用盲注测试

观察响应：检查服务器是否返回了内部资源的内容。

#### 黑盒如何扫描越权漏洞？

1. 识别资源访问点和高权限功能。
2. 修改资源标识符或请求参数，测试权限验证逻辑。
3. 观察服务器的响应，确认是否存在越权漏洞。

#### 黑盒带登录态扫描如何规避业务影响？

+ 数据污染：扫描过程中可能会生成大量测试数据，影响数据库的完整性。
+ 性能影响：大量请求可能导致服务器负载过高，影响正常用户的访问。
+ 业务中断：某些操作（如删除、修改）可能会破坏业务功能。



1. 使用测试环境：在隔离的测试环境中进行扫描。
2. 监控与日志：实时监控服务器性能，记录扫描日志。
3. 测试数据管理：使用测试账号，清理测试数据，避免敏感操作。
4. 配置扫描工具：限制请求速率、禁用危险操作、使用只读模式。
...
1. 安全设计与架构：最小权限、隔离环境、模块化设计。
2. 输入验证与过滤：严格验证输入、过滤恶意内容、使用安全库。
3. 安全通信：加密通信、验证证书、限制网络访问。
4. 漏洞检测与防护：避免自身漏洞、限制扫描范围、禁用危险操作。
5. 日志与监控：记录操作日志、监控异常行为、设置告警。
6. 身份验证与授权：强身份验证、细粒度授权、定期轮换凭证。
7. 更新与维护：定期更新、应用安全补丁、代码审计。
...
1. 规划与设计：安全需求分析、威胁建模、安全架构设计。
2. 开发：安全编码、代码审查、依赖管理。
3. 构建与测试：SAST、DAST、SCA、容器安全扫描。
4. 部署：IaC 安全、配置管理、CI/CD 安全。
5. 运行与监控：RASP、WAF、日志与监控、漏洞管理。
6. 反馈与改进：安全事件响应、持续改进。
7. 文化与培训：安全意识培训、安全文化建设。

#### 落地时候有没有遇到什么困难，如何解决？

增加研发工作量、误报率

外部合规、内部领导、安全培训，工具与流程集成减少人工感知度、减少误报，提高漏洞检测覆盖率。

#### 如何评判devsecops做的好坏，最终我们聚焦到什么上，漏洞闭环等等 

多数安全风险在上线前高效规避，极少软件安全风险可被利用。

#### 工具链的选择理由

自动化、可视化、准确率、商业or开源、二开定制化、poc测试
...
#### 1.代码审计流程，给你一个项目你是如何去审计，审计思路。

1. 初步分析：
   - 目录结构分析： 了解项目的目录结构，识别出核心模块和关键文件。
   - 依赖库分析： 检查项目依赖的第三方库，确认是否存在已知漏洞的版本。
   - 配置文件审查： 检查配置文件（如`application.properties`、`web.xml`等），查找敏感信息泄露或不安全的配置。
2. 代码审计：
   - 入口点分析： 从用户输入点（如HTTP请求、API接口、文件上传等）开始，跟踪数据流。
   - 安全漏洞检测： 针对常见漏洞类型（如SQL注入、XSS、文件上传、反序列化等）进行代码审查。
   - 权限控制检查： 检查权限控制是否合理，是否存在越权访问的风险。
   - 日志与错误处理： 检查日志记录是否完善，错误处理是否合理，避免信息泄露。

**审计思路**

+ 数据流跟踪： 从用户输入点开始，跟踪数据在系统中的流动，检查是否存在未经验证或过滤的数据直接进入敏感操作（如数据库查询、文件操作等）。
+ 安全边界检查： 检查系统的安全边界，确认是否有未受保护的接口或功能。
+ 依赖库安全： 检查项目依赖的第三方库，确认是否存在已知漏洞或不受信任的库。
+ 权限控制： 检查系统的权限控制机制，确认是否存在越权访问的风险。

#### 2.你审计过哪些类型漏洞，你是如何去发现他们的。

sql注入、xss、rce、ssrf等

+ SQL注入： 通过检查数据库查询语句的拼接方式，发现未使用预编译语句或参数化查询的地方。
+ 跨站脚本攻击（XSS）： 检查用户输入是否直接输出到HTML页面，未进行适当的转义或过滤。
+ 文件上传漏洞： 检查文件上传功能，确认是否对文件类型、大小、内容进行了严格的验证。
+ 反序列化漏洞： 检查反序列化操作，确认是否对反序列化的数据进行了严格的验证。
+ 越权访问： 检查权限控制机制，确认是否存在未授权的访问路径或功能。
+ 敏感信息泄露： 检查日志、配置文件、错误信息等，确认是否存在敏感信息泄露的风险



#### 3.文件上传漏洞你一般怎么查找，找哪些方法类名。

查找方法：

查找文件上传功能： 在代码中搜索与文件上传相关的关键字，如MultipartFile、FileUpload、upload等。

检查文件上传处理类： 查找处理文件上传的类或方法，通常这些类会包含文件上传的逻辑。



关键类名和方法：

Spring框架： MultipartFile、CommonsMultipartFile、FileUpload。

Servlet： Part、HttpServletRequest#getPart、HttpServletRequest#getParts。

Apache Commons FileUpload： FileItem、DiskFileItemFactory、ServletFileUpload。



审计要点：

文件类型验证： 检查是否对上传文件的类型进行了严格的验证，避免上传可执行文件或脚本。

文件内容验证： 检查是否对文件内容进行了验证，避免上传恶意文件。

文件存储路径： 检查文件存储路径是否安全，避免文件被直接访问或执行。

文件名处理： 检查文件名是否进行了处理，避免路径遍历或文件名注入。

**文件上传限制了白名单后缀名情况下怎么去从代码层面产生任意文件上传。**

大小写、文件包含，跨路径存储、双后缀、文件内容限制

#### 4.反序列化漏洞你是怎么去审计的举例场景

+ 查找反序列化操作： 在代码中搜索与反序列化相关的关键字，如`ObjectInputStream`、`readObject`、`Serializable`等。
+ 检查反序列化数据源： 确认反序列化的数据来源是否可信，避免反序列化不可信的数据。

一个Java应用从HTTP请求中接收序列化对象，并使用ObjectInputStream进行反序列化。如果未对输入数据进行验证，攻击者可以构造恶意序列化数据，导致远程代码执行。



#### 5.你使用过什么工具，有没有自己写一些自动化审计工具，有没有在工具基础上提升审计效率，以及提升审计效率的思路想法

checkmarx、codeql、semgrep

 根据项目特点，优化工具的配置和规则，减少误报和漏报，提高扫描结果的准确性。



#### 6.逻辑漏洞里越权漏洞你是如何去审计的，权限绕过的具体场景，springmvc认证绕过。

平行越权、垂直越权

通过id请求资源，未校验id与token的关系，是否是自己的资源

#### 7.有没有自己审计过一些rce 如何审计的

runtime.exec()、ProcessBuilder、Process

#### 8.自己有调试过或者研究过哪些著名rce漏洞

springboot、fastjson

#### 9.上传一个jar包放在tomcat里怎么造成rce

反弹shell

1. Tomcat 的类加载机制：
   - Tomcat 会自动加载 `WEB-INF/lib` 目录下的 JAR 包，并将其中的类添加到应用的类路径中。
   - 如果攻击者能够上传恶意 JAR 包到 `WEB-INF/lib` 目录，Tomcat 会加载并执行其中的恶意代码。
2. Servlet 或 Filter 的动态加载：
   - 攻击者可以在恶意 JAR 包中定义 Servlet 或 Filter，并通过 web.xml 配置文件将其注册到应用中。
   - 一旦恶意 Servlet 或 Filter 被加载，攻击者可以通过访问特定的 URL 触发恶意代码的执行。

#### 10.Zip Slip漏洞是如何做到任意文件覆盖，从代码层面。

在解压缩时，程序会读取 ZIP 文件中的文件名，并将其与目标目录拼接，生成最终的文件路径。

如果未对文件名进行严格的验证，攻击者可以通过构造恶意文件名实现路径遍历。



攻击者构造一个恶意 ZIP 文件，其中包含以下条目：

文件名：../../var/www/html/index.jsp

文件内容：恶意 JSP 代码（如 <% Runtime.getRuntime().exec("rm -rf /"); %>）



当程序解压缩该 ZIP 文件时，恶意文件会被写入 /var/www/html/index.jsp，覆盖原有的文件。

攻击者访问 http://target.com/index.jsp，触发恶意代码的执行。
...
'=='是一个比较运算符，用于比较基本数据类型时比较值是否相等，用于引用数据类型时比较的是对象的内存地址是否相同。

'equals()'是Object类的一个方法，默认情况下比较的是引用地址是否相同，但在子类中可以重写以比较对象的实际内容是否相同。

**比较内容：**

'=='在比较基本数据类型时，直接比较值是否相等；在比较引用数据类型时，比较的是对象的内存地址。

'equals()'在默认情况下，比较的是引用地址；如果子类重写了该方法，则可以比较对象的实际内容是否相同。例如，String类重写了'equals()'方法，使得即使两个字符串对象在内存中的地址不同，只要内容相同也会返回true。

**使用场景：**

'=='主要用于基本数据类型的比较和引用类型的地址比较。

'equals()'主要用于对象内容的比较，通常在自定义类中重写以实现特定逻辑的比较。

#### Java虚拟机区域如何划分？

Java 虚拟机（JVM）的内存区域主要分为以下几部分：

+ 方法区（Method Area）：
  - 存储类信息、常量、静态变量、即时编译器编译后的代码等。
  - 在 JDK 8 及之后，方法区被元空间（Metaspace）取代，使用本地内存。
+ 堆区（Heap）：
  - 所有对象实例和数组都在堆上分配内存。
  - 是垃圾收集器管理的主要区域。
+ 栈区（Stack）：
  - 每个线程有一个独立的栈，用于存储局部变量、方法调用和返回值。
  - 栈帧（Stack Frame）是栈的基本单位，对应一次方法调用。
...
+ 方法重载（Overloading）：
  - 在同一个类中，方法名相同但参数列表不同（参数类型、数量或顺序不同）。
  - 返回类型可以相同也可以不同。
  - 编译时多态。
+ 方法重写（Overriding）：
  - 子类重写父类的方法，方法名、参数列表和返回类型必须相同。
  - 访问权限不能比父类更严格，抛出的异常不能比父类更多。
  - 运行时多态。

总结：重载是编译时的多态，重写是运行时的多态。
...
+ `HashMap`：
  ...
  - 性能较高，适用于单线程环境。
+ `HashTable`：
  - 线程安全，使用 `synchronized` 关键字实现同步。
  - 不允许 `null` 作为键和值。
  - 性能较低，适用于多线程环境，但已被 `ConcurrentHashMap` 取代。
+ `ConcurrentHashMap`：
  - 线程安全，采用分段锁（Segment）机制，JDK 8 后使用 CAS 和 `synchronized` 优化。
  - 不允许 `null` 作为键和值。
  - 性能较高，适用于高并发场景。

总结：`HashMap` 适用于单线程，`HashTable` 适用于低并发，`ConcurrentHashMap` 适用于高并发。
...
+ 进程：
  ...
  - 进程间通信（IPC）方式：
    ...
    * 套接字（Socket）
+ 线程：
  - 是操作系统调度的基本单位，共享进程的内存空间。
  - 线程间通信方式：
    * 共享内存（Shared Variables）
    * 锁机制（如 `synchronized`、`ReentrantLock`）
...
+ NIO（Non-blocking I/O）：
  - 同步非阻塞 I/O，基于通道（Channel）和缓冲区（Buffer）。
  - 使用选择器（Selector）实现多路复用。
  - 适用于高并发、连接数较多的场景。
+ AIO（Asynchronous I/O）：
  - 异步非阻塞 I/O，基于事件和回调。
  - 适用于高并发、I/O 操作耗时较长的场景。

总结：BIO 简单但性能低，NIO 性能高但复杂，AIO 性能最高但实现复杂。
...
+ 实现方式：
  1. 懒汉式（Lazy Initialization）：
     * 第一次调用时创建实例。
     * 线程不安全，需加锁（如 `synchronized`）。
  2. 饿汉式（Eager Initialization）：
     * 类加载时创建实例。
     * 线程安全，但可能浪费资源。
  3. 双重检查锁（Double-Checked Locking）：
     * 结合懒汉式和饿汉式的优点，线程安全且性能较高。
  4. 静态内部类：
     * 利用 JVM 的类加载机制实现线程安全的懒加载。
  5. 枚举（Enum）：
     * 最简单的实现方式，线程安全且防止反射和序列化破坏单例。



单例模式是创建型模式，确保全局唯一实例，适用于需要控制实例数量的场景。

### 网络与多线程

#### synchorized如何使用？Object的wait、notify方法有什么作用？

`synchronized` 是 Java 中用于实现线程同步的关键字，可以保证同一时间只有一个线程访问被保护的代码块或方法。
...
+ `wait()`：
  - 使当前线程进入等待状态，并释放锁。
  - 必须在 `synchronized` 块中调用，否则会抛出 `IllegalMonitorStateException`。
  - 调用 `wait()` 后，线程会进入对象的等待队列，直到被唤醒。
+ `notify()`：
  - 唤醒一个等待该对象锁的线程，从等待队列中移除。
  - 被唤醒的线程需要重新竞争锁。
+ `notifyAll()`：
  - 唤醒所有等待该对象锁的线程。
...
+ `sleep()`：
  - 是 `Thread` 类的静态方法，使当前线程暂停执行指定的时间。
  - 不会释放锁，线程仍然持有锁。
  - 适用于需要暂停线程一段时间的场景。
+ `wait()`：
  - 是 `Object` 类的方法，使当前线程进入等待状态，并释放锁。
...
+ 定义：
  - 一个操作或请求，无论执行多少次，结果都是相同的。
  - 例如，HTTP 方法中的 `GET` 是幂等的，因为多次请求不会改变资源状态；而 `POST` 通常不是幂等的，因为多次请求可能会创建多个资源。
+ 适用场景：
  - 防止重复提交（如表单提交、支付请求）。
  - 网络重试（如 RPC 调用、HTTP 请求）。

幂等性是保证多次操作结果一致性的重要特性。

实现方式包括唯一标识、数据库约束、状态机、分布式锁和版本号等。
...
+ 大文件排序：采用外部排序，分块排序后多路归并。



1. 分块排序：
   - 将大文件分割成多个小文件（每个小文件可以完全加载到内存中）。
   - 对每个小文件进行内部排序（如快速排序、归并排序）。
2. 多路归并：
   - 使用多路归并算法将排序后的小文件合并成一个有序的大文件。
   - 每次从每个小文件中读取一个元素，选择最小的元素写入结果文件，直到所有小文件处理完毕。
...
1. 快速选择算法（Quickselect）：
   - 类似于快速排序的分治思想，时间复杂度为 `O(n)`。
   - 通过分区找到第 `k` 小的元素。
2. 堆排序：
   - 使用最大堆或最小堆找到中间值，时间复杂度为 `O(n log n)`。

### 业务基础

#### 调试工具及异常排查流程？

调试工具：IDE 调试工具、日志工具、性能分析工具、网络抓包工具。

```plain
IDE 调试工具：Eclipse、IntelliJ IDEA：提供断点调试、变量监视、堆栈跟踪等功能。
适用于代码级别的调试。

日志工具：Log4j、SLF4J：记录应用程序的运行日志，便于排查问题。
日志级别：DEBUG、INFO、WARN、ERROR。

性能分析工具：JProfiler、VisualVM：分析内存泄漏、CPU 使用率、线程状态等。
适用于性能瓶颈的排查。

网络抓包工具：Wireshark：分析网络请求和响应，排查网络问题。

数据库工具：MySQL Workbench、Navicat：执行 SQL 查询，分析数据库性能。
```

异常排查流程：复现问题、查看日志、分析堆栈跟踪、使用调试工具、检查数据库、网络分析、性能分析、修复问题。
...
检查数据库：
执行 SQL 查询，检查数据一致性和完整性。

网络分析：
使用抓包工具分析网络请求和响应。

性能分析：
使用性能分析工具排查内存泄漏、CPU 瓶颈等问题。
...
故障预防：代码质量、单元测试、集成测试、监控和告警、日志记录、容灾设计、版本控制、灰度发布、定期演练。

#### 数据库索引结构，什么情况下应该建唯一索引？

数据库索引：B+ 树索引、哈希索引、全文索引、空间索引。唯一索引适用于主键、唯一约束和防止重复数据。

1. 主键：
   - 主键必须是唯一的，通常会自动创建唯一索引。
2. 唯一约束：
   - 需要确保某些列的值唯一（如用户邮箱、身份证号）。
3. 防止重复数据：
   - 在业务逻辑中需要确保数据唯一性。

#### 数据库分页语句如何写？

分页查询：使用 LIMIT 和 OFFSET，优化分页查询可以通过主键范围查询和覆盖索引。
...
| Client Hello | 客户端发送支持的 TLS 版本、加密套件、随机数等信息。         |
| Server Hello | 服务器选择加密套件，并发送服务器随机数和证书。              |
| 服务器证书   | 服务器发送数字证书，客户端验证证书的有效性。                |
| 密钥交换     | 客户端和服务器交换密钥交换信息，生成会话密钥。              |
| 完成握手     | 客户端和服务器发送 `Finished`<br/> 消息，验证握手的完整性。 |
| 加密通信     | 使用会话密钥加密和解密数据，确保通信的机密性和完整性。      |
...
1. 客户端使用自签名证书：
   - 抓包工具无法解密自签名证书的流量。
2. 禁用代理自动检测：
   - 客户端禁用代理自动检测，避免流量被抓包工具拦截。
3. 双向认证：
   - 客户端和服务器都提供证书，抓包工具无法完成握手。
4. 使用专用协议：
   - 使用自定义加密协议，抓包工具无法识别或解密流量。
5. 使用 VPN 或专用网络通道：
   - 通过 VPN 或专用网络通道传输数据，绕过抓包工具的拦截。

| 方法               | 原理                            | 优点         | 缺点                 |
| ------------------ | ------------------------------- | ------------ | -------------------- |
| 自签名证书         | 客户端使用自签名证书            | 简单易实现   | 需要客户端信任       |
| 禁用代理自动检测   | 禁用代理自动检测                | 无需修改协议 | 需要定制化配置       |
| 双向认证           | 客户端和服务器都提供证书        | 安全性高     | 配置复杂             |
| 动态证书           | 每次通信生成新证书              | 安全性高     | 实现复杂             |
| 专用协议           | 使用自定义加密协议              | 安全性高     | 开发和维护成本高     |
| TLS 1.3 0-RTT      | 使用 TLS 1.3 的 0-RTT 特性      | 性能优化     | 可能存在重放攻击风险 |
| VPN 或专用网络通道 | 通过 VPN 或专用网络通道传输数据 | 安全性高     | 配置和维护成本高     |
| HTTP/3（QUIC）     | 使用 HTTP/3 和 QUIC 协议        | 性能优化     | 兼容性问题           |
...
客户端（第三方应用）需要在授权服务器上注册，获取 `client_id` 和 `client_secret`。

2. 用户授权

用户授权客户端访问其受保护的资源。

3. 授权码模式（Authorization Code Flow）

通过授权码交换访问令牌，确保安全性。

4. 隐式授权模式（Implicit Flow）

简化授权过程，适用于客户端无法安全存储 `client_secret` 的场景（如浏览器应用）。
...
| **安全风险**             | **描述**                                           | **防范措施**                                   |
| ------------------------ | -------------------------------------------------- | ---------------------------------------------- |
| 授权码泄露               | 授权码在传输过程中被窃取，攻击者可以获取访问令牌。 | 使用 HTTPS，验证 `redirect_uri`<br/>。         |
| CSRF 攻击                | 攻击者伪造请求，诱导用户授权。                     | 使用 `state`<br/> 参数防止 CSRF。              |
| 客户端 ID 和密钥泄露     | 攻击者冒充客户端获取访问令牌。                     | 不要暴露 `client_secret`<br/>，使用安全存储。  |
| 隐式授权模式的安全性问题 | 隐式授权模式直接返回访问令牌，容易被窃取。         | 避免使用隐式授权模式，推荐使用授权码模式。     |
| 访问令牌泄露             | 访问令牌被窃取，攻击者直接访问用户资源。           | 使用短期访问令牌，限制权限范围。               |
| 重放攻击                 | 攻击者重复使用授权码或访问令牌。                   | 使用短期授权码和访问令牌，记录已使用的授权码。 |
| 未验证 `redirect_uri`    | 攻击者伪造回调 URL，窃取授权码或访问令牌。         | 严格验证 `redirect_uri`<br/>，确保其合法。     |
| 权限范围过大             | 客户端请求的权限范围过大，可能导致隐私泄露。       | 请求最小权限范围，用户明确了解权限。           |
| 未使用 PKCE              | 未使用 PKCE 可能导致授权码被窃取。                 | 使用 PKCE 增强安全性。                         |
| 未使用 HTTPS             | 敏感信息在传输过程中被窃取。                       | 确保所有通信都通过 HTTPS 加密传输。            |
...
| **注意事项**     | **描述**                               | **示例代码**                                   |
| ---------------- | -------------------------------------- | ---------------------------------------------- |
| 输入验证与过滤   | 验证 URL 格式，过滤恶意字符。          | `new URL(url)`                                 |
| 防止 SSRF        | 限制请求的目标域名或 IP 范围。         | `isAllowedDomain(url)`                         |
| 使用 HTTPS       | 优先使用 HTTPS，验证 SSL/TLS 证书。    | `HttpsURLConnection`                           |
| 防止重定向滥用   | 限制重定向次数，检查重定向 URL。       | `connection.setInstanceFollowRedirects(false)` |
| 防止 DOS 攻击    | 设置请求超时时间，限制响应内容大小。   | `connection.setConnectTimeout(5000)`           |
| 防止内容注入攻击 | 对响应内容进行消毒处理，防止 XSS。     | `content.replaceAll("<", "&lt;")`              |
| 日志记录与监控   | 记录请求日志，监控异常请求。           | `System.out.println("Request URL: " + url)`    |
| 处理异常情况     | 捕获并处理网络异常和 HTTP 错误状态码。 | `catch (IOException e)`                        |
...
> + 工作层次：
>   - 过滤器工作在 Servlet 容器层面，属于 Java EE 规范的一部分。
>   - 过滤器可以拦截所有请求（包括静态资源和动态资源）。
> + 生命周期：
>   - 过滤器的生命周期由 Servlet 容器管理。
>   - 过滤器在应用启动时初始化，在应用关闭时销毁。
> + 执行顺序：
>   - 过滤器按照配置顺序依次执行。
>   - 可以配置多个过滤器，形成过滤器链。
>
...
>
> + 请求预处理：
>   - 对请求进行预处理（如编码转换、权限验证、日志记录）。
> + 响应后处理：
>   - 对响应进行后处理（如压缩响应、添加响应头）。
> + 统一处理：
>   - 统一处理跨域请求、CSRF 防护、XSS 过滤等。
...
> + 工作层次：
>   - 拦截器工作在 Spring 框架层面，属于 Spring MVC 的一部分。
>   - 拦截器只能拦截 Spring MVC 的请求（即 Controller 处理的请求）。
> + 生命周期：
>   - 拦截器的生命周期由 Spring 容器管理。
>   - 拦截器在应用启动时初始化，在应用关闭时销毁。
> + 执行顺序：
>   - 拦截器按照配置顺序依次执行。
>   - 可以配置多个拦截器，形成拦截器链。
>
...
+ 适用于对所有请求进行统一处理（如编码转换、跨域处理、CSRF 防护）。
+ 工作在 Servlet 容器层面，拦截范围更广。

拦截器：

+ 适用于对 Spring MVC 请求进行处理（如权限验证、日志记录、性能监控）。
+ 工作在 Spring 框架层面，功能更强大。
...
| **特性**     | **SESSION**                       | **Token**                         | **Cookie**                                       |
| ------------ | --------------------------------- | --------------------------------- | ------------------------------------------------ |
| **存储位置** | 服务器端                          | 客户端（如 localStorage、Cookie） | 客户端（浏览器）                                 |
| **存储大小** | 理论上无限制，受服务器内存限制    | 通常较小（如 JWT 最大 8KB）       | 每个 Cookie 最大 4KB，每个域名最多 50 个         |
| **生命周期** | 默认关闭浏览器后失效，可配置延长  | 可设置过期时间，支持刷新机制      | 可设置过期时间，分为会话和持久 Cookie            |
| **安全性**   | SESSION ID 需要保护，防止会话劫持 | Token 需要保护，防止泄露和篡改    | 可通过 `HttpOnly`<br/> 和 `Secure`<br/> 标志增强 |
| **应用场景** | 保存用户登录状态、临时数据        | 无状态身份验证、跨域身份验证      | 保存用户偏好设置、跟踪用户行为                   |
| **跨域支持** | 不支持跨域                        | 支持跨域                          | 支持跨域，但受同源策略限制                       |
| **无状态性** | 有状态                            | 无状态                            | 有状态                                           |


+ SESSION：
  - 适用于需要保存用户状态和临时数据的场景（如购物车、表单数据）。
  - 适合传统的 Web 应用。
+ Token：
  - 适用于无状态的身份验证和跨域场景（如 RESTful API、单点登录）。
  - 适合前后端分离的应用和移动端应用。
+ Cookie：
  - 适用于保存用户偏好设置和跟踪用户行为。
  - 适合简单的身份验证（如记住登录状态）。
...
通过设置 Cookie 的 `HttpOnly` 标志来实现。`HttpOnly` 标志是 Cookie 的一个属性，用于限制 JavaScript 访问 Cookie，从而提高安全性。

在设置 SESSION ID 的 Cookie 时，添加 `HttpOnly` 标志。防止xss

限制 Cookie 的作用域和路径，防止跨站请求伪造（CSRF）攻击。

| **措施**             | **作用**                                                     | **示例代码**                                                 |
| -------------------- | ------------------------------------------------------------ | ------------------------------------------------------------ |
| `HttpOnly`<br/> 标志 | 防止 JavaScript 读取 SESSION ID，防止 XSS 攻击。             | `sessionCookie.setHttpOnly(true);`                           |
| `Secure`<br/> 标志   | 仅在 HTTPS 连接中发送 Cookie，防止 SESSION ID 在 HTTP 连接中泄露。 | `sessionCookie.setSecure(true);`                             |
| 设置作用域和路径     | 限制 Cookie 的作用域和路径，防止 CSRF 攻击。                 | `sessionCookie.setPath("/"); sessionCookie.setDomain("example.com");` |
| 定期更换 SESSION ID  | 定期更换 SESSION ID，防止会话固定攻击。                      | `session.invalidate(); session = request.getSession(true);`  |
| 使用 CSRF Token      | 防止跨站请求伪造（CSRF）攻击。                               | `session.setAttribute("csrfToken", generateCsrfToken());`    |
...
| **步骤**     | **描述**                                              | **示例代码**                                                 |
| ------------ | ----------------------------------------------------- | ------------------------------------------------------------ |
| 生成 Token   | 使用安全的随机数生成器生成 Token。                    | `SecureRandom`<br/> + `Base64`                               |
| 存储 Token   | 将 Token 存储在 SESSION 中。                          | `session.setAttribute("csrfToken", csrfToken);`              |
| 嵌入 Token   | 将 Token 嵌入到表单或请求头中。                       | `<input type="hidden" name="csrfToken" value="${csrfToken}">` |
| 验证 Token   | 在服务器端验证 Token 是否与 SESSION 中的 Token 一致。 | `sessionToken != null && sessionToken.equals(requestToken)`  |
| 设置过期时间 | 为 Token 设置过期时间，防止 Token 被长期滥用。        | `session.setAttribute("csrfTokenExpiry", expiryTime);`       |
...
同源策略是浏览器的一种安全机制，用于限制不同源（Origin）之间的资源访问。同源策略的核心是确保一个源的文档或脚本只能访问与其同源的资源，防止恶意网站窃取用户数据。



| **技术**    | **原理**                          | **适用场景**       | **安全性**             |
| ----------- | --------------------------------- | ------------------ | ---------------------- |
| CORS        | 服务器声明允许跨域访问            | AJAX 请求          | 高（需配置正确）       |
| JSONP       | 利用 `<script>`<br/> 标签跨域请求 | 简单的 GET 请求    | 低（易受 XSS 攻击）    |
| 代理服务器  | 通过同源代理服务器转发请求        | 隐藏目标服务器地址 | 高（可过滤和验证请求） |
| WebSocket   | 全双工通信协议，不受同源策略限制  | 实时通信           | 高（需加密和身份验证） |
| PostMessage | 窗口间安全传递消息                | 跨域窗口通信       | 高（需验证消息来源）   |
